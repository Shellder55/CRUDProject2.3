...
public class AdminController {
    ...
    public String allUsers(Model model, Principal principal) {
        log.info("От '{}', получен запрос на загрузку всех пользователей.", principal.getName());
    ...
    public String getUser(@PathVariable(value = "id") Long id, Model model, Principal principal) {
        log.info("От '{}', получен запрос на просмотр пользователя. ID пользователя: {}", principal.getName(), id);
    ...
    public String newUser(@ModelAttribute("user") User user, Model model, Principal principal) {
        log.info("От '{}', получен запрос на создание пользователя", principal.getName());
    ...
    public String editUsers(@PathVariable(value = "id") Long id, Model model, Principal principal) {
        log.info("От '{}', получен запрос на редактирование пользователя. ID пользователя: {}", principal.getName(), id);
    ...
    public String deleteUsers(@PathVariable(value = "id") Long id, Principal principal) {
        log.info("От '{}', получен запрос на удаление пользователя. ID пользователя: {}", principal.getName(), id);
